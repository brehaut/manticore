...
(function (Manticore) {
    (function (Bestiary) {
        var rawdata = [
            ["Giant ant", 0, "normal", "troop", ["animal"]],
        ...
        var Monster = (function () {
            function Monster(name, level, size, kind, attributes) {
                this.name = name;
                this.level = level;
                this.size = size;
                this.kind = kind;
        ...
        var PricedMonster = (function (_super) {
            ...
            function PricedMonster(name, level, size, kind, attributes, price) {
                _super.call(this, name, level, size, kind, attributes);
                this.price = price;
            }
        ...
        function monsterFromRecord(record) {
            return new Monster(record[0], record[1], record[2], record[3], record[4]);
        }
        ...
        function priceMonster(partyLevel, m) {
            ...
            return new PricedMonster(m.name, m.level, m.size, m.kind, m.attributes, cost * multiplier);
        }
        ...
        function allocateMonsters(points, monsters) {
            var allAllocations = [];
            var allowedUnspent = Math.min.apply(null, monsters.map(function (m) {
                return m.price;
            }));

            function allocate(remainingPoints, monstersIdx, acc) {
                // cap results at a maximum.
                if (allAllocations.length >= 5000)
                    throw { message: "Results truncated" };
        ...
        // public API:
        Bestiary.monsters = rawdata.map(monsterFromRecord);

        function allocationsForParty(characters, partyLevel, selectedMonsters) {
            return allocateMonsters(priceParty(characters), selectedMonsters.map(function (m) {
                return priceMonster(partyLevel, m);
            }).filter(function (m) {
                return m !== null;
            }));
        }
        Bestiary.allocationsForParty = allocationsForParty;
    })(Manticore.Bestiary || (Manticore.Bestiary = {}));
    var Bestiary = Manticore.Bestiary;
})(Manticore || (Manticore = {}));
